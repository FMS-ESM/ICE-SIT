...
public          &
        ...
        nfields, Aerosol_time, Model_init_time, Aerosol_column_time
...
!--------------------------------------------------------------------
          if (use_aerosol_timeseries) then
            ...
            call print_date(Aerosol_time(1))
...
!!! bjt >> 
!!!           call print_date (model_time,  str= &
!!!               ' model_time: ')
...
#ifdef test_aerosol      
      print *, "do_column_aerosol=",do_column_aerosol
#endif
...
!----------------------------------------------------------------------
#ifdef test_aerosol
         print *, "do_specified_aerosol=",do_specified_aerosol
#endif
         if ( do_specified_aerosol) then
...
!--------------------------------------------------------------------
#ifdef test_aerosol
              print *, "aerosol_data_source=",trim(aerosol_data_source)
#endif
              if (trim(aerosol_data_source) == 'calculate_column') then
...
!--------------------------------------------------------------------
                ...
                if (make_separate_calls) then                
                  do n=1,nfields
#ifdef test_aerosol
                    print *, "Aerosol_time(",n,"):"
                    call print_date(Aerosol_time(n))
#endif
...
#ifdef test_aerosol
                     print *, "aerosol_name:",Aerosol%aerosol_names(n)
                     print *, "aerosol=",Aerosol%aerosol(:,:,:,n)
#endif
                  end do  !(nfields)
...
#ifdef test_aerosol
                   print *, "aerosol_name:",Aerosol%aerosol_names(1)
                   print *, "aerosol=",Aerosol%aerosol(:,:,:,:)
#endif
                endif      
              endif ! (calculate_column)
...
#ifdef test_aerosol
          print *, "override_aerosols=",override_aerosols
#endif
...
!              call data_override('ATM', TRIM(tracer_names(n))//'_aerosol',&
!                                 aerosol_proc, Model_init_time, override=used)
              if (used) then
...
real :: latb(NLON+1,NLAT+1),lonb(NLON+1,NLAT+1),pi,phalf(NLON,NLAT,NLEV+1),p_flux(NLON,NLAT,NLEV+1)
...
do i = 1,NLEV+1
   ...
   p_flux(:,:,i) = REAL(i-1)/REAL(NLEV)   
end do

call aerosol_init (lonb, latb, aerosol_names, aerosol_family_names)
!call aerosol_init (lonb, latb, names, aerosol_family_names)
...
do i =0,14
  ...
  call aerosol_time_vary (model_time)
  call print_date (Aerosol_time(1),  str= 'Aerosol_time: ')
  call aerosol_driver (1,1,model_time, tracer, phalf, p_flux, Aerosol,.true.)
enddo
...
!!! if (mpp_pe() == mpp_root_pe() ) then
!!!   print *, "aerosol_names=",aerosol_names
!!!   print *, "aerosol=",Aerosol%aerosol(:,:,:,:)  
!!! endif
