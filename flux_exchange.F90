...
!bjt
use sit_vdiff_mod,  only: xmissing
...
private
  ...
  public :: flux_exchange_init,   &
     ...
     flux_exchange_end                 ! bjt, 20141103

!-----------------------------------------------------------------------
  character(len=128) :: version = '$Id: flux_exchange.F90,v 20.0.2.1.2.1 2014/02/28 16:32:50 Seth.Underwood Exp $'
...
!----- all fields will be output on the atmospheric grid -----
        ...
        call diag_field_init ( Time, Atm%axes(1:2), Land%axes, Ice%axes )
...
!allocate atmos_ice_boundary
        ...
        allocate( atmos_ice_boundary%sw_flux(is:ie,js:je,kd) )            ! bjt
        ...
        allocate( atmos_ice_boundary%fluxn(is:ie,js:je,kd) )              ! bjt
        allocate( atmos_ice_boundary%dfluxn(is:ie,js:je,kd) )             ! bjt
        allocate( atmos_ice_boundary%lprec(is:ie,js:je,kd) )
        allocate( atmos_ice_boundary%fprec(is:ie,js:je,kd) )

        allocate( atmos_ice_boundary%tprec(is:ie,js:je,kd) )              ! bjt
        allocate( atmos_ice_boundary%gust(is:ie,js:je,kd) )               ! bjt
...
! initialize boundary values for override experiments (mjh)
        ...
        atmos_ice_boundary%sw_flux=0.0                                    ! bjt
        ...
        atmos_ice_boundary%fluxn=0.0                                      ! bjt
        atmos_ice_boundary%dfluxn=0.0                                     ! bjt
        atmos_ice_boundary%lprec=0.0
        atmos_ice_boundary%fprec=0.0

        atmos_ice_boundary%tprec=xmissing        ! bjt
        atmos_ice_boundary%gust=xmissing        ! bjt
...
! ------------------------------------------------
!
subroutine flux_exchange_end ( atmos_ice_boundary, land_ice_atmos_boundary, &
       land_ice_boundary, ice_ocean_boundary, ocean_ice_boundary )
  type(atmos_ice_boundary_type),     intent(inout) :: atmos_ice_boundary
  type(land_ice_atmos_boundary_type),intent(inout) :: land_ice_atmos_boundary
  type(land_ice_boundary_type),      intent(inout) :: land_ice_boundary
  type(ice_ocean_boundary_type),     intent(inout) :: ice_ocean_boundary
  type(ocean_ice_boundary_type),     intent(inout) :: ocean_ice_boundary
  character(len=64), parameter    :: sub_name = 'flux_exchange_end'
  character(len=256), parameter   :: note_header = '==>Note from ' // trim(module_name) //     &
                                                   '(' // trim(sub_name) // '):'
!-----------------------------------------------------------------------

  deallocate(tr_table)
  deallocate(tr_table_map)

!--------- read gridspec file ------------------
!only atmos pelists needs to do it here, ocean model will do it elsewhere
  deallocate( ice_pelist ) !if ice/land become concurrent, this won't be true...
  deallocate( ocn_pelist )
        
!deallocate atmos_ice_boundary
  deallocate( atmos_ice_boundary%u_flux)
  deallocate( atmos_ice_boundary%v_flux)
  deallocate( atmos_ice_boundary%u_star)
  deallocate( atmos_ice_boundary%t_flux)
  deallocate( atmos_ice_boundary%q_flux)
  deallocate( atmos_ice_boundary%lw_flux)
  deallocate( atmos_ice_boundary%sw_flux)            ! bjt
  deallocate( atmos_ice_boundary%sw_flux_vis_dir)
  deallocate( atmos_ice_boundary%sw_flux_vis_dif)
  deallocate( atmos_ice_boundary%sw_flux_nir_dir)
  deallocate( atmos_ice_boundary%sw_flux_nir_dif)
  deallocate( atmos_ice_boundary%fluxn)              ! bjt
  deallocate( atmos_ice_boundary%dfluxn)             ! bjt
  deallocate( atmos_ice_boundary%lprec)
  deallocate( atmos_ice_boundary%fprec)

  deallocate( atmos_ice_boundary%tprec)              ! bjt
  deallocate( atmos_ice_boundary%gust)               ! bjt
   
  deallocate( atmos_ice_boundary%dhdt)
  deallocate( atmos_ice_boundary%dedt)
  deallocate( atmos_ice_boundary%drdt)
  deallocate( atmos_ice_boundary%coszen)
  deallocate( atmos_ice_boundary%p)

!deallocate land_ice_boundary
  deallocate( land_ice_boundary%runoff)
  deallocate( land_ice_boundary%calving)
  deallocate( land_ice_boundary%runoff_hflx)
  deallocate( land_ice_boundary%calving_hflx)

!deallocate land_ice_atmos_boundary
  deallocate( land_ice_atmos_boundary%t)
  deallocate( land_ice_atmos_boundary%albedo)
  deallocate( land_ice_atmos_boundary%albedo_vis_dir)
  deallocate( land_ice_atmos_boundary%albedo_nir_dir)
  deallocate( land_ice_atmos_boundary%albedo_vis_dif)
  deallocate( land_ice_atmos_boundary%albedo_nir_dif)
  deallocate( land_ice_atmos_boundary%land_frac)
  deallocate( land_ice_atmos_boundary%dt_t)
  deallocate( land_ice_atmos_boundary%dt_tr)
  deallocate( land_ice_atmos_boundary%u_flux)
  deallocate( land_ice_atmos_boundary%v_flux)
  deallocate( land_ice_atmos_boundary%dtaudu)
  deallocate( land_ice_atmos_boundary%dtaudv)
  deallocate( land_ice_atmos_boundary%u_star)
  deallocate( land_ice_atmos_boundary%b_star)
  deallocate( land_ice_atmos_boundary%q_star)
  deallocate( land_ice_atmos_boundary%rough_mom)
  deallocate( land_ice_atmos_boundary%frac_open_sea)

!deallocate ocean_ice_boundary
  deallocate( ocean_ice_boundary%u)
  deallocate( ocean_ice_boundary%v)
  deallocate( ocean_ice_boundary%t)
  deallocate( ocean_ice_boundary%s)

!frazil and sea_level are optional, if not present they should be nullified
  deallocate( ocean_ice_boundary%frazil)
  deallocate( ocean_ice_boundary%sea_level)

  deallocate( ice_ocean_boundary%u_flux)
  deallocate( ice_ocean_boundary%v_flux)
  deallocate( ice_ocean_boundary%t_flux)
  deallocate( ice_ocean_boundary%q_flux)
  deallocate( ice_ocean_boundary%salt_flux)
  deallocate( ice_ocean_boundary%lw_flux)
  deallocate( ice_ocean_boundary%sw_flux_vis_dir)
  deallocate( ice_ocean_boundary%sw_flux_vis_dif)
  deallocate( ice_ocean_boundary%sw_flux_nir_dir)
  deallocate( ice_ocean_boundary%sw_flux_nir_dif)
  deallocate( ice_ocean_boundary%lprec)
  deallocate( ice_ocean_boundary%fprec)
  deallocate( ice_ocean_boundary%runoff)
  deallocate( ice_ocean_boundary%calving)
  deallocate( ice_ocean_boundary%runoff_hflx)
  deallocate( ice_ocean_boundary%calving_hflx)
  deallocate( ice_ocean_boundary%p)
  deallocate( ice_ocean_boundary%mi)
...
!! bjt >>
!!!!  call data_override ('ICE', 'ice_mask',     Ice%ice_mask,      Time)
!  call data_override ('ICE', 'obsseaice',     Ice%obsseaice,      Time)
!  call data_override ('ICE', 'obswtb',     Ice%obswtb,      Time)
!  call data_override ('ICE', 'obswsb',     Ice%obswsb,      Time)
!! << bjt
...
!!!             ! 2-d new common vars
!!!   !!! if (id_siced > 0) used = send_data(id_siced, Ice%siced(:,:), Ice%Time, mask=Ice%mask)      ! bjt, seaice thickness in water equivalent
!!!   if (id_obsseaice > 0) used = send_data(id_obsseaice, Ice%obsseaice, Ice%Time, mask=Ice%mask)     ! bjt
!!!   if (id_obswtb > 0) used = send_data(id_obswtb, Ice%obswtb, Ice%Time, mask=Ice%mask)              ! bjt
!!!   if (id_obswsb > 0) used = send_data(id_obswsb, Ice%obswsb, Ice%Time, mask=Ice%mask)              ! bjt
!!!             ! 2-d SIT/ECHAM vars
!!!   if (id_cor > 0)    used = send_data(id_cor, Ice%cor, Ice%Time, mask=Ice%mask)                    ! bjt
!!!   if (id_slm > 0)    used = send_data(id_slm, Ice%slm, Ice%Time, mask=Ice%mask)                    ! bjt
!!!   if (id_lclass > 0)    used = send_data(id_lclass, Ice%lclass, Ice%Time, mask=Ice%mask)           ! bjt
!!!   if (id_sitmask > 0)    used = send_data(id_sitmask, Ice%sitmask, Ice%Time, mask=Ice%mask)        ! bjt
!!!   if (id_bathy > 0)    used = send_data(id_bathy, Ice%bathy, Ice%Time, mask=Ice%mask)              ! bjt
!!!   if (id_wlvl > 0)    used = send_data(id_wlvl, Ice%wlvl, Ice%Time, mask=Ice%mask)                 ! bjt
!!!   if (id_ocnmask > 0)    used = send_data(id_ocnmask, Ice%ocnmask, Ice%Time, mask=Ice%mask)        ! bjt
!!!   if (id_obox_mask > 0)    used = send_data(id_obox_mask, Ice%obox_mask, Ice%Time, mask=Ice%mask)  ! bjt
!!!   if (id_sni > 0)    used = send_data(id_sni, Ice%sni, Ice%Time, mask=Ice%mask)                    ! bjt
!!!   if (id_siced > 0)    used = send_data(id_siced, Ice%siced, Ice%Time, mask=Ice%mask)                    ! bjt
!!!   if (id_tsi > 0)    used = send_data(id_tsi, Ice%tsi, Ice%Time, mask=Ice%mask)                    ! bjt
!!!   if (id_tsw > 0)    used = send_data(id_tsw, Ice%tsw, Ice%Time, mask=Ice%mask)                    ! bjt
!!!   if (id_tsl > 0)    used = send_data(id_tsl, Ice%tsl, Ice%Time, mask=Ice%mask)                    ! bjt
!!!   if (id_tslm > 0)    used = send_data(id_tslm, Ice%tslm, Ice%Time, mask=Ice%mask)                 ! bjt
!!!   if (id_tslm1 > 0)    used = send_data(id_tslm1, Ice%tslm1, Ice%Time, mask=Ice%mask)              ! bjt
!!!   if (id_ocu > 0)    used = send_data(id_ocu, Ice%ocu, Ice%Time, mask=Ice%mask)                    ! bjt
!!!   if (id_ocv > 0)    used = send_data(id_ocv, Ice%ocv, Ice%Time, mask=Ice%mask)                    ! bjt
!!!   if (id_ctfreez2 > 0)    used = send_data(id_ctfreez2, Ice%ctfreez2, Ice%Time, mask=Ice%mask)     ! bjt
!!!             ! 2-d SIT vars
!!!   if (id_wtb > 0)    used = send_data(id_wtb, Ice%wtb, Ice%Time, mask=Ice%mask)                    ! bjt
!!!   if (id_wub > 0)    used = send_data(id_wub, Ice%wub, Ice%Time, mask=Ice%mask)                    ! bjt
!!!   if (id_wvb > 0)    used = send_data(id_wvb, Ice%wvb, Ice%Time, mask=Ice%mask)                    ! bjt
!!!   if (id_wsb > 0)    used = send_data(id_wsb, Ice%wsb, Ice%Time, mask=Ice%mask)                    ! bjt
!!!   if (id_fluxiw > 0)    used = send_data(id_fluxiw, Ice%fluxiw, Ice%Time, mask=Ice%mask)           ! bjt
!!!   if (id_pme2 > 0)    used = send_data(id_pme2, Ice%pme2, Ice%Time, mask=Ice%mask)                 ! bjt
!!!   if (id_subfluxw > 0)    used = send_data(id_subfluxw, Ice%subfluxw, Ice%Time, mask=Ice%mask)     ! bjt
!!!   if (id_wsubsal > 0)    used = send_data(id_wsubsal, Ice%wsubsal, Ice%Time, mask=Ice%mask)        ! bjt
!!!   if (id_cc > 0)    used = send_data(id_cc, Ice%cc, Ice%Time, mask=Ice%mask)                       ! bjt
!!!   if (id_hc > 0)    used = send_data(id_hc, Ice%hc, Ice%Time, mask=Ice%mask)                       ! bjt
!!!   if (id_engwac > 0)    used = send_data(id_engwac, Ice%engwac, Ice%Time, mask=Ice%mask)           ! bjt
!!!   if (id_sc > 0)    used = send_data(id_sc, Ice%sc, Ice%Time, mask=Ice%mask)                       ! bjt
!!!   if (id_saltwac > 0)    used = send_data(id_saltwac, Ice%saltwac, Ice%Time, mask=Ice%mask)        ! bjt
!!!   if (id_wtfns > 0)    used = send_data(id_wtfns, Ice%wtfns, Ice%Time, mask=Ice%mask)              ! bjt
!!!   if (id_wsfns > 0)    used = send_data(id_wsfns, Ice%wsfns, Ice%Time, mask=Ice%mask)              ! bjt
!!!             ! 3-d SIT vars: snow/ice
!!!   if (id_zsi > 0)    used = send_data(id_zsi, Ice%zsi, Ice%Time)                    ! bjt
!!!   if (id_silw > 0)    used = send_data(id_silw, Ice%silw, Ice%Time)                 ! bjt
!!!   if (id_tsnic > 0)    used = send_data(id_tsnic, Ice%tsnic, Ice%Time)              ! bjt
!!!             ! 3-d SIT vars: water column
!!!   if (id_obswt > 0)    used = send_data(id_obswt, Ice%obswt, Ice%Time)              ! bjt
!!!   if (id_obsws > 0)    used = send_data(id_obsws, Ice%obsws, Ice%Time)              ! bjt
!!!   if (id_obswu > 0)    used = send_data(id_obswu, Ice%obswu, Ice%Time)              ! bjt
!!!   if (id_obswv > 0)    used = send_data(id_obswv, Ice%obswv, Ice%Time)              ! bjt
!!!   if (id_wt > 0)    used = send_data(id_wt, Ice%wt, Ice%Time)                       ! bjt
!!!   if (id_wu > 0)    used = send_data(id_wu, Ice%wu, Ice%Time)                       ! bjt
!!!   if (id_wv > 0)    used = send_data(id_wv, Ice%wv, Ice%Time)                       ! bjt
!!!   if (id_ww > 0)    used = send_data(id_ww, Ice%ww, Ice%Time)                       ! bjt
!!!   if (id_ws > 0)    used = send_data(id_ws, Ice%ws, Ice%Time)                       ! bjt
!!!   if (id_wtke > 0)    used = send_data(id_wtke, Ice%wtke, Ice%Time)                 ! bjt
!!!   if (id_wlmx > 0)    used = send_data(id_wlmx, Ice%wlmx, Ice%Time)                 ! bjt
!!!   if (id_wldisp > 0)    used = send_data(id_wldisp, Ice%wldisp, Ice%Time)           ! bjt
!!!   if (id_wkm > 0)    used = send_data(id_wkm, Ice%wkm, Ice%Time)                    ! bjt
!!!   if (id_wkh > 0)    used = send_data(id_wkh, Ice%wkh, Ice%Time)                    ! bjt
!!!   if (id_wrho1000 > 0)    used = send_data(id_wrho1000, Ice%wrho1000, Ice%Time)     ! bjt
!!!   if (id_wtfn > 0)    used = send_data(id_wtfn, Ice%wtfn, Ice%Time)                 ! bjt
!!!   if (id_wsfn > 0)    used = send_data(id_wsfn, Ice%wsfn, Ice%Time)                 ! bjt
!!!   if (id_awufl > 0)    used = send_data(id_awufl, Ice%awufl, Ice%Time)              ! bjt
!!!   if (id_awvfl > 0)    used = send_data(id_awvfl, Ice%awvfl, Ice%Time)              ! bjt
!!!   if (id_awtfl > 0)    used = send_data(id_awtfl, Ice%awtfl, Ice%Time)              ! bjt
!!!   if (id_awsfl > 0)    used = send_data(id_awsfl, Ice%awsfl, Ice%Time)              ! bjt
!!!   if (id_awtkefl > 0)    used = send_data(id_awtkefl, Ice%awtkefl, Ice%Time)        ! bjt
!!!   if (id_awufl > 0)    used = send_data(id_awufl, Ice%awufl, Ice%Time)              ! bjt
!!!   if (id_awufl > 0)    used = send_data(id_awufl, Ice%awufl, Ice%Time)              ! bjt
!!!   if (id_awufl > 0)    used = send_data(id_awufl, Ice%awufl, Ice%Time)              ! bjt
!!!   if (id_awufl > 0)    used = send_data(id_awufl, Ice%awufl, Ice%Time)              ! bjt
!!!
!!!    ! 4- OUTPUT only, original ATM variabels
!!!   if (id_seaice > 0)    used = send_data(id_seaice, Ice%seaice, Ice%Time)              ! bjt
!!!   if (id_grndcapc > 0)    used = send_data(id_grndcapc, Ice%grndcapc, Ice%Time)              ! bjt
!!!   if (id_grndhflx > 0)    used = send_data(id_grndhflx, Ice%grndhflx, Ice%Time)              ! bjt
!!!   if (id_grndflux > 0)    used = send_data(id_grndflux, Ice%grndflux, Ice%Time)              ! bjt
...
#ifdef DEBUG
    if (GDCHK0) print *, "sfc_boundary_layer: before surface_flux"
    if (GDCHK0) print *, "Ice%t_surf=",Ice%t_surf
    if (GDCHK0) print *, "ex_t_surf=",ex_t_surf
    if (GDCHK0) print *, "ex_t_ca=",ex_t_ca
#endif
...
subroutine flux_down_from_atmos (Time, Atm, Land, Ice, &
 ...
 !!!      ex_t_bot, &  temperature at lowest model level    ! bjt
...
!!!  call data_override ('ATM', 't_bot',    Atm%t_bot,   Time)                   ! bjt,  It is ALREADY in line 1340
...
!!!    if (GDCHK0) print *, "flux_down_from_atmos: before put_to_xgrid"
!!!    if (GDCHK0) print *, "Atm%flux_sw_dir=",Atm%flux_sw_dir
!!!    if (GDCHK0) print *, "Atm%flux_sw_vis_dir=",Atm%flux_sw_vis_dir
!!!    if (GDCHK0) print *, "Atm%flux_sw_dif=",Atm%flux_sw_dif
!!!    if (GDCHK0) print *, "Atm%flux_sw_vis_dif=",Atm%flux_sw_vis_dif
...
!!$  else
     ...
     call put_to_xgrid (Atm%gust,   'ATM', ex_gust, xmap_sfc, complete=.false.)
!!$  endif
...
!!!    if (GDCHK0) then
!!!      print *, "flux_down_from_atmos: after put_to_xgrid, but before ex comp. sw1way_bug=",sw1way_bug
!!!      print *, "ex_albedo_vis_dir_fix=",ex_albedo_vis_dir_fix      
!!!      print *, "i ","ex_flux_sw_vis_dir ","ex_albedo_vis_dir_fix ","ex_flux_sw_vis_dir * ex_albedo_vis_dir_fix"
!!!      do i= 1, n_xgrid_sfc
!!!        !!! if (GDCHK0) print *, "ex_flux_sw_dir=",ex_flux_sw_dir
!!!        !!! if (GDCHK0) print *, "ex_flux_sw_dif=",ex_flux_sw_dif
!!!        print *, i, ex_flux_sw_vis_dir(i),ex_albedo_vis_dir_fix(i),ex_flux_sw_vis_dir(i) * ex_albedo_vis_dir_fix(i)
!!!      enddo
!!!    endif
...
!!!    if (GDCHK0) print *, "flux_down_from_atmos: after put_to_xgrid and ex comp. sw1way_bug=",sw1way_bug
!!!    !!! if (GDCHK0) print *, "ex_flux_sw_dir=",ex_flux_sw_dir
!!!    if (GDCHK0) print *, "ex_albedo_vis_dir_fix=",ex_albedo_vis_dir_fix
!!!    if (GDCHK0) print *, "ex_flux_sw_vis_dir=",ex_flux_sw_vis_dir
!!!    !!! if (GDCHK0) print *, "ex_flux_sw_vis_dif=",ex_flux_sw_vis_dif
...
!!! call get_from_xgrid (Land_boundary%t_bot,   'LND', ex_t_bot,     xmap_sfc)    ! bjt
...
!!!   call data_override('LND', 't_bot',   Land_boundary%t_bot,   Time )
...
!!!    if (GDCHK0) print *, "flux_ice_to_ocean: before get_from_xgrid"
!!!    if (GDCHK0) print *, "Ice_boundary%sw_flux_vis_dir=",Ice_boundary%sw_flux_vis_dir(:,JJ,:)
!!!    if (GDCHK0) print *, "Ice_boundary%sw_flux_vis_dif=",Ice_boundary%sw_flux_vis_dif(:,JJ,:)
!!!    if (GDCHK0) print *, "Ice_boundary%sw_flux_nir_dir=",Ice_boundary%sw_flux_nir_dir(:,JJ,:)
!!!    if (GDCHK0) print *, "Ice_boundary%sw_flux_nir_dif=",Ice_boundary%sw_flux_nir_dif(:,JJ,:)
  ...
  call get_from_xgrid (Ice_boundary%tprec,    'OCN', ex_tprec,     xmap_sfc)  ! bjt
  call get_from_xgrid (Ice_boundary%gust,     'OCN', ex_gust,      xmap_sfc)  ! bjt

!!!    if (GDCHK0) print *, "flux_ice_to_ocean: after get_from_xgrid"
!!!    if (GDCHK0) print *, "Ice_boundary%sw_flux_vis_dir=",Ice_boundary%sw_flux_vis_dir(:,JJ,:)
!!!    if (GDCHK0) print *, "Ice_boundary%sw_flux_vis_dif=",Ice_boundary%sw_flux_vis_dif(:,JJ,:)
!!!    if (GDCHK0) print *, "Ice_boundary%sw_flux_nir_dir=",Ice_boundary%sw_flux_nir_dir(:,JJ,:)
!!!    if (GDCHK0) print *, "Ice_boundary%sw_flux_nir_dif=",Ice_boundary%sw_flux_nir_dif(:,JJ,:)
...
!!!    if (GDCHK0) print *, "flux_ice_to_ocean: before data_override"
!!!    if (GDCHK0) print *, "Ice_boundary%sw_flux_vis_dir=",Ice_boundary%sw_flux_vis_dir(:,JJ,:)
!!!    if (GDCHK0) print *, "Ice_boundary%sw_flux_vis_dif=",Ice_boundary%sw_flux_vis_dif(:,JJ,:)
!!!    if (GDCHK0) print *, "Ice_boundary%sw_flux_nir_dir=",Ice_boundary%sw_flux_nir_dir(:,JJ,:)
!!!    if (GDCHK0) print *, "Ice_boundary%sw_flux_nir_dif=",Ice_boundary%sw_flux_nir_dif(:,JJ,:)  
...
!!!    if (GDCHK0) print *, "flux_ice_to_ocean: after data_override"
!!!    if (GDCHK0) print *, "Ice_boundary%sw_flux_vis_dir=",Ice_boundary%sw_flux_vis_dir(:,JJ,:)
...
!!!    if (GDCHK0) print *, "flux_ice_to_ocean: after data_override and ov=",ov
!!!    if (GDCHK0) print *, "Ice%albedo_vis_dir=",Ice%albedo_vis_dir(:,JJ,:)
!!!    if (GDCHK0) print *, "Ice_boundary%sw_flux_vis_dir=",Ice_boundary%sw_flux_vis_dir(:,JJ,:)
...
!!!    if (GDCHK0) print *, "flux_ice_to_ocean: after data_override 2"
!!!    if (GDCHK0) print *, "Ice_boundary%sw_flux_vis_dir=",Ice_boundary%sw_flux_vis_dir(:,JJ,:)
!!!    if (GDCHK0) print *, "Ice_boundary%sw_flux_vis_dif=",Ice_boundary%sw_flux_vis_dif(:,JJ,:)
!!!    if (GDCHK0) print *, "Ice_boundary%sw_flux_nir_dir=",Ice_boundary%sw_flux_nir_dir(:,JJ,:)
!!!    if (GDCHK0) print *, "Ice_boundary%sw_flux_nir_dif=",Ice_boundary%sw_flux_nir_dif(:,JJ,:)  
...
!!!    if (GDCHK0) print *, "flux_ice_to_ocean: before stock_move"
!!!    if (GDCHK0) print *, "Ice_boundary%sw_flux_vis_dir=",Ice_boundary%sw_flux_vis_dir(:,JJ,:)
!!!    if (GDCHK0) print *, "Ice_boundary%sw_flux_vis_dif=",Ice_boundary%sw_flux_vis_dif(:,JJ,:)
!!!    if (GDCHK0) print *, "Ice_boundary%sw_flux_nir_dir=",Ice_boundary%sw_flux_nir_dir(:,JJ,:)
!!!    if (GDCHK0) print *, "Ice_boundary%sw_flux_nir_dif=",Ice_boundary%sw_flux_nir_dif(:,JJ,:)
...
!!!    if (GDCHK0) print *, "flux_ice_to_ocean: after stock_move"
!!!    if (GDCHK0) print *, "Ice_boundary%sw_flux_vis_dir=",Ice_boundary%sw_flux_vis_dir(:,JJ,:)
!!!    if (GDCHK0) print *, "Ice_boundary%sw_flux_vis_dif=",Ice_boundary%sw_flux_vis_dif(:,JJ,:)
!!!    if (GDCHK0) print *, "Ice_boundary%sw_flux_nir_dir=",Ice_boundary%sw_flux_nir_dir(:,JJ,:)
!!!    if (GDCHK0) print *, "Ice_boundary%sw_flux_nir_dif=",Ice_boundary%sw_flux_nir_dif(:,JJ,:)       
...
!                time step (Kg/m2)
!        t_bot = temperature at lowest model level since last
!                time step (K?)
...
!!!    if (GDCHK0) print *, "flux_ice_to_ocean: before flux_ice_to_ocean_redistribute"
!!!    if (GDCHK0) print *, "Ice_Ocean_Boundary%sw_flux_nir_dir=",Ice_Ocean_Boundary%sw_flux_nir_dir(:,JJ)
!!!    if (GDCHK0) print *, "Ice_Ocean_Boundary%sw_flux_nir_dif=", Ice_Ocean_Boundary%sw_flux_nir_dif(:,JJ)
!!!    if (GDCHK0) print *, "Ice_Ocean_Boundary%sw_flux_vis_dir=",Ice_Ocean_Boundary%sw_flux_vis_dir(:,JJ)
!!!    if (GDCHK0) print *, "Ice_Ocean_Boundary%sw_flux_vis_dif=",Ice_Ocean_Boundary%sw_flux_vis_dif(:,JJ)
...
!!!    if (GDCHK0) print *, "flux_ice_to_ocean: after flux_ice_to_ocean_redistribute"
!!!    if (GDCHK0) print *, "Ice_Ocean_Boundary%sw_flux_nir_dir=",Ice_Ocean_Boundary%sw_flux_nir_dir(:,JJ)
!!!    if (GDCHK0) print *, "Ice_Ocean_Boundary%sw_flux_nir_dif=", Ice_Ocean_Boundary%sw_flux_nir_dif(:,JJ)
!!!    if (GDCHK0) print *, "Ice_Ocean_Boundary%sw_flux_vis_dir=",Ice_Ocean_Boundary%sw_flux_vis_dir(:,JJ)
!!!    if (GDCHK0) print *, "Ice_Ocean_Boundary%sw_flux_vis_dif=",Ice_Ocean_Boundary%sw_flux_vis_dif(:,JJ)      
...
!!!  if(ASSOCIATED(Ice_Ocean_Boundary%t_bot) ) call flux_ice_to_ocean_redistribute( Ice, Ocean, &            ! bjt
!!!      Ice%fprec, Ice_Ocean_Boundary%t_bot Ice_Ocean_Boundary%xtype, do_area_weighted_flux )
...
subroutine diag_field_init ( Time, atmos_axes, land_axes, ice_axes )

  type(time_type), intent(in) :: Time
  integer,         intent(in) :: atmos_axes(2)
  integer,         intent(in) :: land_axes(2)
  integer,         intent(in) :: ice_axes(2)
...
!!!  ! sit
!!!  !!! id_siced      = &
!!!  !!! register_diag_field ( mod_name, 'siced',      ice_axes, Time, &
!!!  !!!                      'ice thickness (swe)',  'm', &
!!!  !!!                      missing_value=xmissing )
!!!  id_obsseaice  = &
!!!  register_diag_field ( mod_name, 'obsseaice',      ice_axes, Time, &
!!!                       'observed sea ice fraction',  '0-1', &
!!!                       missing_value=xmissing )
!!!  id_obswtb  = &
!!!  register_diag_field ( mod_name, 'obswtb',      ice_axes, Time, &
!!!                       'observed sea surface temperature',  'K', &
!!!                       missing_value=xmissing )
!!!  id_obswsb  = &
!!!  register_diag_field ( mod_name, 'obswsb',      ice_axes, Time, &
!!!                       'observed sea surface salinity',  'PSU', &
!!!                       missing_value=xmissing )
!!!  id_sni      = &
!!!  register_diag_field ( mod_name, 'sni',      ice_axes, Time, &
...
!!!                       
!!!  id_tsi      = &
!!!  register_diag_field ( mod_name, 'tsi',      ice_axes, Time, &
!!!                       'surface temperature of ice',  'K', &
!!!                       missing_value=xmissing )
!!!  id_tsw      = &
!!!  register_diag_field ( mod_name, 'tsw',      ice_axes, Time, &
!!!                       'skin temperatrue over water ',  'K', &
!!!                       missing_value=xmissing )
!!!  id_seaice      = &
!!!  register_diag_field ( mod_name, 'seaice',      ice_axes, Time, &
!!!                       'ice cover: fraction of 1-SLM',  &
!!!                       ' (0-1)', &
!!!                       missing_value=xmissing )
!!!  id_ocu      = &
!!!  register_diag_field ( mod_name, 'ocu',      ice_axes, Time, &
!!!                       'ocean eastw. velocity',  'm/s', &
!!!                       missing_value=xmissing )
!!!  id_ocv      = &
!!!  register_diag_field ( mod_name, 'ocv',      ice_axes, Time, &
...
!!!  !!! if (.false.) then
!!!    id_cor      = &
!!!    register_diag_field ( mod_name, 'cor',      ice_axes, Time, &
!!!                         'corlios factor 2*omega*sin(lat)',  '1/s', &
!!!                         missing_value=xmissing )
!!!    id_slm      = &
!!!    register_diag_field ( mod_name, 'slm',      ice_axes, Time, &
!!!                         'pslm: land fraction',  '0-1', &
!!!                         missing_value=xmissing )
!!!    id_lclass      = &
!!!    register_diag_field ( mod_name, 'lclass',      ice_axes, Time, &
!!!                         'land cover class, 1: land, 2: ocean, 3: lake, 4: glacier',  '1-4', &
!!!                         missing_value=xmissing )
!!!    id_sitmask      = &
!!!    register_diag_field ( mod_name, 'sitmask',      ice_axes, Time, &
!!!                         'mask for sit(1=.TRUE., 0=.FALSE.)',  '0/1', &
!!!                         missing_value=xmissing )
!!!    
!!!    id_bathy      = &
!!!    register_diag_field ( mod_name, 'bathy',      ice_axes, Time, &
!!!                         'bathymeter (topography or orography) of ocean',  'm', &
!!!                         missing_value=xmissing )
!!!                         
!!!    id_wlvl      = &
!!!    register_diag_field ( mod_name, 'wlvl',      ice_axes, Time, &
!!!                         'current water level (ice/water interface) a water body grid',  'm', &
!!!                         missing_value=xmissing )
!!!                         
!!!    id_ocnmask      = &
!!!    register_diag_field ( mod_name, 'ocnmask',      ice_axes, Time, &
!!!                         'mask for 3-D ocean grid',  '1: ocn', &
!!!                         missing_value=xmissing )
!!!                         
!!!    id_obox_mask      = &
!!!    register_diag_field ( mod_name, 'obox_mask',      ice_axes, Time, &
...
!!!!                         
!!!    id_tsl      = &
!!!    register_diag_field ( mod_name, 'tsl',      ice_axes, Time, &
!!!                         'calcuated Earth skin temperature at t+dt',  'K', &
!!!                         missing_value=xmissing )
!!!    id_tslm      = &
!!!    register_diag_field ( mod_name, 'tslm',      ice_axes, Time, &
!!!                         'calcuated Earth skin temperature at t',  'K', &
!!!                         missing_value=xmissing )
!!!    id_tslm1      = &
!!!    register_diag_field ( mod_name, 'tslm1',      ice_axes, Time, &
...
!!!!                         missing_value=xmissing )
!!!    id_ctfreez2      = &
!!!    register_diag_field ( mod_name, 'ctfreez2',      ice_axes, Time, &
!!!                         'ref water freezing temperature',  'K', &
!!!                         missing_value=xmissing )
!!!    ! 2-d SIT vars                                          I
!!!    id_wtb      = &
!!!    register_diag_field ( mod_name, 'wtb',      ice_axes, Time, &
!!!                         'bulk water temperature',  'K', &
!!!                         missing_value=xmissing )
!!!    id_wub      = &
!!!    register_diag_field ( mod_name, 'wub',      ice_axes, Time, &
!!!                         'bulk water u current',  'm/s', &
!!!                         missing_value=xmissing )
!!!    id_wvb      = &
!!!    register_diag_field ( mod_name, 'wvb',      ice_axes, Time, &
!!!                         'bulk water v current',  'm/s', &
!!!                         missing_value=xmissing )
!!!    id_wsb      = &
!!!    register_diag_field ( mod_name, 'wsb',      ice_axes, Time, &
!!!                         'bulk water salinity',  'PSU', &
!!!                         missing_value=xmissing )
!!!    id_fluxiw      = &
!!!    register_diag_field ( mod_name, 'fluxiw',      ice_axes, Time, &
!!!                         'over-water net surface heat flux',  'W/m2, + upward, i.e., from ocean', &
!!!                         missing_value=xmissing )
!!!    
!!!    id_pme2      = &
!!!    register_diag_field ( mod_name, 'pme2',      ice_axes, Time, &
!!!                         'net fresh water into ocean (P-E+ice_corr)',  'm/s, + downward', &
!!!                         missing_value=xmissing )
!!!    id_subfluxw      = &
!!!    register_diag_field ( mod_name, 'subfluxw',      ice_axes, Time, &
!!!                         'subsurface ocean heat flux',  'W/m2, + upward', &
!!!                         missing_value=xmissing )
!!!    id_wsubsal      = &
!!!    register_diag_field ( mod_name, 'wsubsal',      ice_axes, Time, &
!!!                         'subsurface ocean salinity flux',  'm*PSU/s, + upward', &
!!!                         missing_value=xmissing )
!!!    id_cc      = &
!!!    register_diag_field ( mod_name, 'cc',      ice_axes, Time, &
...
!!!                         missing_value=xmissing )
!!!    id_hc      = &
!!!    register_diag_field ( mod_name, 'hc',      ice_axes, Time, &
!!!                         'heat content per water fraction (ice sheet+openwater), energy of a water column above tmelt', &
!!!                         'J/m2', &
!!!                         missing_value=xmissing )
!!!    id_engwac      = &
!!!    register_diag_field ( mod_name, 'engwac',      ice_axes, Time, &
...
!!!    
!!!    id_sc      = &
!!!    register_diag_field ( mod_name, 'sc',      ice_axes, Time, &
!!!                         'salinity content per water fraction (ice sheet+openwater)', &
!!!                         'PSU*m', &
!!!                         missing_value=xmissing )
!!!    id_saltwac      = &
!!!    register_diag_field ( mod_name, 'saltwac',      ice_axes, Time, &
!!!                         'accumulated salt into water fraction',  &
!!!                         'PSU*m, + downward', &
!!!                         missing_value=xmissing )
!!!    
!!!    id_wtfns      = &
!!!    register_diag_field ( mod_name, 'wtfns',      ice_axes, Time, &
!!!                         'nudging flux into sit-ocean for an entire column, i.e., pwtfns=SUM(pwtfn(icol,:))',  &
!!!                         'W/m**2', &
!!!                         missing_value=xmissing )
!!!    id_wsfns      = &
!!!    register_diag_field ( mod_name, 'wsfns',      ice_axes, Time, &
...
!!!    
!!!    id_zsi      = &
!!!    register_diag_field ( mod_name, 'zsi',      axt_snowice2, Time, &
!!!                         'dry snow (level=0)/ice (level=0) water equivalent,',  &
!!!                         'm', &
!!!                         missing_value=xmissing )
!!!    id_silw      = &
!!!    register_diag_field ( mod_name, 'silw',      axt_snowice2, Time, &
!!!                         'snow (level=0)/ice (level=0) liquid water storage',  &
!!!                         'm', &
!!!                         missing_value=xmissing )
!!!    id_tsnic      = &
!!!    register_diag_field ( mod_name, 'tsnic',      axt_snowice4, Time, &
...
!!!    
!!!    id_obswt      = &
!!!    register_diag_field ( mod_name, 'obswt',      axt_water, Time, &
!!!                         'observed potentail water temperature',  &
!!!                         'K', &
!!!                         missing_value=xmissing )
!!!    id_obsws      = &
!!!    register_diag_field ( mod_name, 'obsws',      axt_water, Time, &
!!!                         'observed salinity',  &
!!!                         'PSU, 0/00', &
!!!                         missing_value=xmissing )
!!!    id_obswu      = &
!!!    register_diag_field ( mod_name, 'obswu',      axt_water, Time, &
!!!                         'observed u-current',  &
!!!                         'm/s', &
!!!                         missing_value=xmissing )
!!!    id_obswv      = &
!!!    register_diag_field ( mod_name, 'obswv',      axt_water, Time, &
!!!                         'observed v-current',  &
!!!                         'm/s', &
!!!                         missing_value=xmissing )
!!!    id_wt      = &
!!!    register_diag_field ( mod_name, 'wt',      axt_water, Time, &
!!!                         'potential water temperature',  &
!!!                         'K', &
!!!                         missing_value=xmissing )
!!!    id_wu      = &
!!!    register_diag_field ( mod_name, 'wu',      axt_water, Time, &
!!!                         'u current',  &
!!!                         'm/s', &
!!!                         missing_value=xmissing )
!!!    id_wv      = &
!!!    register_diag_field ( mod_name, 'wv',      axt_water, Time, &
!!!                         'v current',  &
!!!                         'm/s', &
!!!                         missing_value=xmissing )
!!!    id_ww      = &
!!!    register_diag_field ( mod_name, 'ww',      axt_waterf, Time, &
!!!                         'w current',  &
!!!                         'm/s', &
!!!                         missing_value=xmissing )
!!!    id_ws      = &
!!!    register_diag_field ( mod_name, 'ws',      axt_water, Time, &
!!!                         'practical salinity',  &
!!!                         '0/00', &
!!!                         missing_value=xmissing )
!!!    id_wtke      = &
!!!    register_diag_field ( mod_name, 'wtke',      axt_waterf, Time, &
!!!                         'turbulent kinetic energy',  &
!!!                         'm2/s2', &
!!!                         missing_value=xmissing )
!!!    id_wlmx      = &
!!!    register_diag_field ( mod_name, 'wlmx',      axt_waterf, Time, &
!!!                         'mixing length',  &
!!!                         'm', &
!!!                         missing_value=xmissing )
!!!    id_wldisp      = &
!!!    register_diag_field ( mod_name, 'wldisp',      axt_waterf, Time, &
!!!                         'dissipation length',  &
!!!                         'm', &
!!!                         missing_value=xmissing )
!!!    id_wkm      = &
!!!    register_diag_field ( mod_name, 'wkm',      axt_waterf, Time, &
!!!                         'eddy diffusivity for momentum',  &
!!!                         'm2/s', &
!!!                         missing_value=xmissing )
!!!    id_wkh      = &
!!!    register_diag_field ( mod_name, 'wkh',      axt_waterf, Time, &
!!!                         'eddy diffusivity for heat',  &
!!!                         'm2/s', &
!!!                         missing_value=xmissing )
!!!    id_wrho1000      = &
!!!    register_diag_field ( mod_name, 'wrho1000',      axt_water, Time, &
!!!                         '1000-m potential water density',  &
!!!                         'kg/m3', &
!!!                         missing_value=xmissing )
!!!    id_wtfn      = &
!!!    register_diag_field ( mod_name, 'wtfn',      axt_water, Time, &
!!!                         'mean temperature flux nudging at each level (positive into ocean)',  &
!!!                         'K/s', &
!!!                         missing_value=xmissing )
!!!    id_wsfn      = &
!!!    register_diag_field ( mod_name, 'wsfn',      axt_water, Time, &
!!!                         'mean salinity flux nudging at each level (positive into ocean)',  &
!!!                         'PSU/s', &
!!!                         missing_value=xmissing )
!!!    id_awufl      = &
!!!    register_diag_field ( mod_name, 'awufl',      axt_water, Time, &
!!!                         'advected u flux at each level (positive into ocean)',  &
!!!                         'm/s2', &
!!!                         missing_value=xmissing )
!!!    id_awvfl      = &
!!!    register_diag_field ( mod_name, 'awvfl',      axt_water, Time, &
!!!                         'advected v flux at each level (positive into ocean)',  &
!!!                         'm/s2', &
!!!                         missing_value=xmissing )
!!!    id_awtfl      = &
!!!    register_diag_field ( mod_name, 'awtfl',      axt_water, Time, &
...
!!!                         missing_value=xmissing )
!!!    id_awtkefl      = &
!!!    register_diag_field ( mod_name, 'awtkefl',      axt_waterf, Time, &
!!!                         'advected tke at each level (positive into ocean)',  &
!!!                         'm2/s3', &
!!!                         missing_value=xmissing )
!!!!  !  final output only
!!!!    id_seaice      = &
!!!!    register_diag_field ( mod_name, 'seaice',      axt, Time, &
!!!!                         'ice cover: fraction of 1-SLM',  &
!!!!                         ' (0-1)', &
!!!!                         missing_value=xmissing )
!!!  !  implicit with vdiff
!!!    id_grndcapc      = &
!!!    register_diag_field ( mod_name, 'grndcapc',      axt, Time, &
!!!                         'areal heat capacity of the uppermost sit layer (snow/ice/water)',  &
!!!                         'J/m**2/K', &
!!!                         missing_value=xmissing )
!!!    id_grndhflx      = &
!!!    register_diag_field ( mod_name, 'grndhflx',      axt, Time, &
!!!                         'ground heat flux below the surface',  &
!!!                         'W/m**2, + upward, into the skin layer', &
!!!                         missing_value=xmissing )
!!!    id_grndflux      = &
!!!    register_diag_field ( mod_name, 'grndflux',      axt, Time, &
...
!    Ice%t_bot     temperature at lowest model level    ! bjt
