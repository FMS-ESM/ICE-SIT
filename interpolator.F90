...
! Modified by Benjei Tsuang (btsuang@gmail.com) (2015/1/11)
!  1) case insenitive for netcdf filednames
...
integer                  :: level_type    ! Pressure, Sigma or Depth level
integer                  :: is,ie,js,je
...
! Flags to indicate where climatology pressure levels are pressure, sigma or depth levels
integer, parameter :: PRESSURE = 1, SIGMA = 2 , DEPTH = 3
...
!!!                               vert_interp, interp_method, clim_units, single_year_file)
...
character(len=*), intent(in) , optional :: interp_method        ! bjt
!++lwh
...
! index nlev at highest pressure.
    ...
    case('depth')
      ...
      allocate(clim_type%levs(nlev))
      call mpp_get_axis_data(axes(i),clim_type%levs)
      clim_type%level_type = DEPTH
  ! Convert to m
      if( chomp(units) == "cm") then
         clim_type%levs = clim_type%levs * 100.
      end if
! define the direction of the vertical data axis
! switch index order if necessary so that indx 1 is at lowest depth,
! index nlev at highest depth.
      if( sense == 1 ) then
        clim_type%vertical_indices = INCREASING_UPWARD
          allocate (alpha(nlev))
          do n = 1, nlev
...
!!!       clim_type%climatological_year = (fileyr == 0)
       clim_type%climatological_year = ((fileyr == 0).or.(fileyr == 1))
...
! -------------------------------------------------------------------
    ...
    if( .not. associated(clim_type%halflevs) )  then
        ...
        else if (clim_type%level_type == DEPTH   ) then  ! extrapolation, bjt
          clim_type%halflevs(nlev+1) = clim_type%levs(nlev)+0.5*(clim_type%levs(nlev)- &
                                         clim_type%levs(nlev-1))
        else                   ! extrapolation, bjt
          clim_type%halflevs(nlev+1) = clim_type%levs(nlev)+0.5*(clim_type%levs(nlev)- &
                                         clim_type%levs(nlev-1))
        endif
        do n=2,nlev
...
!Assume that the horizontal interpolation within a file is the same for each variable.
  ...
  if (present(interp_method)) then
    if(trim(interp_method) == "conservative") then
      ...
      call horiz_interp_new (clim_type%interph, &
                          clim_type%lonb, clim_type%latb, &
                          agrid_mod(:,:,1), agrid_mod(:,:,2), interp_method=trim(interp_method))
    endif
  else
...
! Only read the fields named in data_names
   do j=1,size(data_names(:))
      ...
      do i=1,nvar
         ...
         if( lowercase(name) == lowercase(data_names(j)) ) then
            units=chomp(units)
...
jend = jstart - 1 + size(interp_data,2)
  ...
  do i= 1,size(clim_type%field_name(:))
!!++lwh
   if ( lowercase(field_name) == lowercase(clim_type%field_name(i)) ) then
!--lwh
    found_field=.true.
    exit 
 endif
...
!We have the previous time step but not the next time step data
  ...
  if (GDCHK1) then
    print *,"interpolator_4D 1.0: clim_type%data(taum,1)=",clim_type%data(ID_LON,ID_LAT,:,1,1)
    print *,"interpolator_4D 1.0: clim_type%data(taup,1)=",clim_type%data(ID_LON,ID_LAT,:,2,1)
    print *,"interpolator_4D 1.0: clim_type%data(taum,2)=",clim_type%data(ID_LON,ID_LAT,:,1,2)
    print *,"interpolator_4D 1.0: clim_type%data(taup,2)=",clim_type%data(ID_LON,ID_LAT,:,2,2)
    print *,"interpolator_4D 2.0: clim_type%TIME_FLAG=",clim_type%TIME_FLAG
    print *,"interpolator_4D 2.0: clim_type%tweight=",clim_type%tweight
    print *,"interpolator_4D 2.0: clim_type%tweight1=",clim_type%tweight1
    print *,"interpolator_4D 2.0: clim_type%tweight2=",clim_type%tweight2
    print *,"interpolator_4D 2.0: clim_type%tweight3=",clim_type%tweight3
  endif
...
select case(clim_type%TIME_FLAG)
  case (LINEAR)
    do i=1, size(clim_type%field_name(:))
      where (clim_type%data(:,:,:,clim_type%itaum,i).ne.xmissing) 
        where (clim_type%data(:,:,:,clim_type%itaup,i).ne.xmissing)
          !!! hinterp_data(:,:,:,i) =  xmissing 
          !!! hinterp_data(:,:,:,i) = (1.-clim_type%tweight)*clim_type%data(:,:,:,clim_type%itaum,i) &
          !!!              + clim_type%tweight*clim_type%data(:,:,:,clim_type%itaup,i)
          hinterp_data(:,:,:,i) = (1.-clim_type%tweight)*clim_type%data(:,:,:,clim_type%itaum,i) &
                        + clim_type%tweight*clim_type%data(:,:,:,clim_type%itaup,i)
        elsewhere
         !!!  hinterp_data(:,:,:,i) =  xmissing
            !!!          clim_type%data(:,:,:,clim_type%itaum,i)
           interp_data(:,:,:,i)=clim_type%data(:,:,:,clim_type%itaum,i)
        endwhere
      elsewhere
        where (clim_type%data(:,:,:,clim_type%itaup,i).ne.xmissing) 
          !!! hinterp_data(:,:,:,i) = xmissing
          hinterp_data(:,:,:,i) = clim_type%data(:,:,:,clim_type%itaup,i)
        elsewhere
          hinterp_data(:,:,:,i) =  xmissing
        endwhere
      endwhere
...
! Do sine fit to data at this point
  case (BILINEAR)
    do i=1, size(clim_type%field_name(:))
      where (clim_type%pmon_pyear(:,:,:,i).ne.xmissing) 
        where (clim_type%nmon_pyear(:,:,:,i).ne.xmissing) 
          hinterp_data(:,:,:,i) = &
            (1.-clim_type%tweight1) * (1.-clim_type%tweight3) * clim_type%pmon_pyear(:,:,:,i) + &
            (1.-clim_type%tweight2) *     clim_type%tweight3  * clim_type%nmon_pyear(:,:,:,i) + &
                 clim_type%tweight1 * (1.-clim_type%tweight3) * clim_type%pmon_nyear(:,:,:,i) + &
                 clim_type%tweight2 *     clim_type%tweight3  * clim_type%nmon_nyear(:,:,:,i)
        elsewhere
          hinterp_data(:,:,:,i) =  &
                      clim_type%pmon_pyear(:,:,:,i)
        endwhere
  ...
  if (GDCHK1) then
    print *,"interpolator_4D 2.0: hinterp_data(1)=",hinterp_data(ID_LON,ID_LAT,:,1)
    print *,"interpolator_4D 2.0: clim_type%data(itaum,1)=",clim_type%data(ID_LON,ID_LAT,:,clim_type%itaum,1)
    print *,"interpolator_4D 2.0: clim_type%data(itaup,1)=",clim_type%data(ID_LON,ID_LAT,:,clim_type%itaup,1)
    print *,"interpolator_4D 2.0: hinterp_data(2)=",hinterp_data(ID_LON,ID_LAT,:,2)
    print *,"interpolator_4D 2.0: clim_type%data(itaum,2)=",clim_type%data(ID_LON,ID_LAT,:,clim_type%itaum,2)
    print *,"interpolator_4D 2.0: clim_type%data(itaup,2)=",clim_type%data(ID_LON,ID_LAT,:,clim_type%itaup,2)
    print *,"interpolator_4D 3.0: clim_type%level_type=",clim_type%level_type
  endif
  ...
  select case(clim_type%mr(i))
    case(NO_CONV)
      do k = 1,size(hinterp_data,3)
        where (hinterp_data(:,:,k,i).ne.xmissing)
          where (col_data(:,:,i).ne.xmissing)
            col_data(:,:,i) = col_data(:,:,i) + hinterp_data(:,:,k,i)* &
              (clim_type%halflevs(k+1)-clim_type%halflevs(k))/grav
          elsewhere
            col_data(:,:,i) = hinterp_data(:,:,k,i)* &
              (clim_type%halflevs(k+1)-clim_type%halflevs(k))/grav
          endwhere
        endwhere
      enddo
...
enddo
  ...
  if (GDCHK1) then
    print *,"interpolator_4D 3.0: hinterp_data(1)=",hinterp_data(ID_LON,ID_LAT,:,1)
    print *,"interpolator_4D 3.0: hinterp_data(2)=",hinterp_data(ID_LON,ID_LAT,:,2)
  endif
...
enddo
  ...
  if (GDCHK1) then
    print *,"interpolator_4D 4.0: vert_interp(1)=",clim_type%vert_interp(1)
    print *,"interpolator_4D 4.0: interp_data(1)=",interp_data(ID_LON,ID_LAT,:,1)
    print *,"interpolator_4D 4.0: vert_interp(2)=",clim_type%vert_interp(2)
    print *,"interpolator_4D 4.0: interp_data(2)=",interp_data(ID_LON,ID_LAT,:,2)
  endif
!--lwh
do i= 1, size(clim_type%field_name(:))
  select case(clim_type%mr(i))
    case(KG_M2)
      do k = 1,size(interp_data,3)
         interp_data(:,:,k,i) = MERGE(interp_data(:,:,k,i)*(phalf(:,:,k+1)-phalf(:,:,k)),xmissing,interp_data(:,:,k,i).ne.xmissing)
      enddo
  end select
end do

  if (GDCHK1) then
    print *,"interpolator_4D 5.0: clim_type%mr(1)=",clim_type%mr(1)
    print *,"interpolator_4D 5.0: interp_data(1)=",interp_data(ID_LON,ID_LAT,:,1)
    print *,"interpolator_4D 5.0: clim_type%mr(2)=",clim_type%mr(2)
    print *,"interpolator_4D 5.0: interp_data(2)=",interp_data(ID_LON,ID_LAT,:,2)
  endif
...
!We have the previous time step but not the next time step data
  ...
  if (GDCHK1) then
    print *,"interpolator_3D 1.0: clim_type%itaum=",clim_type%itaum
    print *,"interpolator_3D 1.0: clim_type%itaup=",clim_type%itaup
    print *,"interpolator_3D 1.0: clim_type%data(1,1)=",clim_type%data(ID_LON,ID_LAT,:,1,1)
    print *,"interpolator_3D 1.0: clim_type%data(2,1)=",clim_type%data(ID_LON,ID_LAT,:,2,1)
    print *,"interpolator_3D 1.0: clim_type%data(1,2)=",clim_type%data(ID_LON,ID_LAT,:,1,2)
    print *,"interpolator_3D 1.0: clim_type%data(2,2)=",clim_type%data(ID_LON,ID_LAT,:,2,2)
  endif
...
select case(clim_type%TIME_FLAG)
  case (LINEAR)
    where (clim_type%data(:,:,:,clim_type%itaum,i).ne.xmissing) 
      where (clim_type%data(:,:,:,clim_type%itaup,i).ne.xmissing) 
        hinterp_data = (1.-clim_type%tweight)*clim_type%data(:,:,:,clim_type%itaum,i) &
                     + clim_type%tweight*clim_type%data(:,:,:,clim_type%itaup,i)
      elsewhere
        hinterp_data =  &
                    clim_type%data(:,:,:,clim_type%itaum,i)
    ...
    elsewhere
      where (clim_type%data(:,:,:,clim_type%itaup,i).ne.xmissing) 
        hinterp_data = &
                    clim_type%data(:,:,:,clim_type%itaup,i)
      elsewhere
          hinterp_data =  xmissing
      endwhere
    endwhere  
! case (SEASONAL)
! Do sine fit to data at this point
  ...
  if (GDCHK1) then
    print *,"interpolator_3D 1.0: clim_type%data=",hinterp_data(ID_LON,ID_LAT,:)
  endif
...
select case(clim_type%mr(i))
  ...
  if (GDCHK1) then
    print *,"interpolator_3D 3.0: hinterp_data=",hinterp_data(ID_LON,ID_LAT,:)
  endif
...
enddo
  ...
  if (GDCHK1) then
    print *,"interpolator_3D 4.0: interp_data=",interp_data(ID_LON,ID_LAT,:)
  endif
...
select case(clim_type%mr(i))
  case(KG_M2)
    do k = 1,size(interp_data,3)
       interp_data(:,:,k) = MERGE(interp_data(:,:,k)*(phalf(:,:,k+1)-phalf(:,:,k)),xmissing,interp_data(:,:,k).ne.xmissing)
    enddo
end select
...
enddo !End of i loop
  ...
  if (GDCHK1) then
    print *,"interpolator_3D 5.0: interp_data=",interp_data(ID_LON,ID_LAT,:)
  endif
...
do n=1, size(clim_type%field_name(:))
  hinterp_data(:,:,:,n) = clim_type%data(:,:,:,1,n)
end do
...
do i= 1,size(clim_type%field_name(:))
  if ( field_name == clim_type%field_name(i) ) then
    ...
    hinterp_data = clim_type%data(:,:,:,1,i)
...
!!! #ifdef test_woa
      print *, "mpp_read:"
      print *, "climdata=",climdata(ID_LON,ID_LAT,:)
!!! #endif
   endif
...
!  level data. the indices themselves were previously flipped.
      ...
      call mpp_get_atts(src_field,missing=missing)
      allocate(mask_in(size(clim_type%lon(:)),size(clim_type%lat(:)), &
                        size(clim_type%levs(:))))         ! bjt
      mask_in=merge(1.,0.,climdata.NE.missing)   ! bjt

!!!      climdata=merge(climdata,0.,climdata.NE.missing)   ! bjt
!!! #ifdef test_woa
!!!       print *, "after merge missing:"
!!!       print *, "climdata=",climdata(:,:,1)
!!! #endif

      climdata=merge(climdata,xmissing,climdata.NE.missing)   ! set missing to default xmissing,  bjt
  if (GDCHK1) then
!!! #ifdef test_woa
      print *, "after merge missing:"
      print *, "climdata=",climdata(ID_LON,ID_LAT,1)

!!! #endif
   endif
      call horiz_interp(clim_type%interph, climdata, hdata, missing_value=xmissing, mask_in=mask_in)
      !!! call horiz_interp_new(clim_type%interph, climdata, hdata, missing_value=missing, mask_in=mask_in)
      !!! hdata=merge(hdata,xmissing,hdata.NE.missing)   ! set missing to default xmissing,  bjt
  if (GDCHK1) then
!!! #ifdef test_woa
      print *, "after horiz_interp:"
      print *, "hdata=",hdata(ID_LON,ID_LAT,:)
!!! #endif
  endif
...
!  level data. the indices themselves were previously flipped.
      ...
      !!! call horiz_interp(clim_type%interph, climdata, hdata, missing_value=xmissing)

      call mpp_get_atts(src_field,missing=missing)
      allocate(mask_in(size(clim_type%lon(:)),size(clim_type%lat(:)), &
                        size(clim_type%levs(:))))         ! bjt
      mask_in=merge(1.,0.,climdata.NE.missing)   ! bjt      
      climdata=merge(climdata,xmissing,climdata.NE.missing)   ! bjt
      call horiz_interp(clim_type%interph, climdata, hdata, missing_value=xmissing, mask_in=mask_in)
      ! call horiz_interp(clim_type%interph, climdata, hdata, missing_value=missing, mask_in=mask_in)
      !!! hdata=merge(hdata,xmissing,hdata.NE.missing)   ! set missing to default xmissing,  bjt
      deallocate(climdata)
      deallocate(mask_in)
...
!!! subroutine interp_weighted_scalar_2D (grdin, grdout, datin, datout, verbose, mask_in, mask_out, &
!!!      missing_value, missing_permit, err_msg  )
   !-----------------------------------------------------------------------
   !   overload of interface interp_weighted_scalar_2D
   !   uses 2d arrays for data and mask
   !   this allows for multiple interpolations with one call
   !-----------------------------------------------------------------------
...
!!!   ! << bjt
   ...
   if (size(datout,2).ne.size(datin,2)) then
     call mpp_error(FATAL,'interp_weighted_scalar : input data and output data do not have the nlon grids')
   else
     nlon=size(datout,2)
   endif
#ifdef interp_weighted_scalar_2D
   print *, "grdin=",grdin
   print *, "grdout=",grdout
#endif
   do k = 1, size(datout,1 )
     datout(k,:) = xmissing
#ifdef interp_weighted_scalar_2D
     print *, "interp_weighted_scalar_2D: 2.0:"
     print *, "2.0: ","k","j","datout(k,:)"
#endif
     do j = 1, size(datin,1 )
       if ( grdin(j)   <= grdout(k) .and. &
         ...
         n=1
         where (datin(j,:).ne.xmissing)
           where (datout(k,:).eq.xmissing)
             datout(k,:) = datin(j,:)*(grdin(j+1)-grdout(k))
           elsewhere
             datout(k,:) = datout(k,:) + datin(j,:)*(grdin(j+1)-grdout(k))
           endwhere
         endwhere
#ifdef interp_weighted_scalar_2D
         print *, "2.1: ",k,j,n,"(grdin(j+1)-grdout(k))=",(grdin(j+1)-grdout(k))
         print *, "2.1: ",k,j,n,datin(j,:)
         print *, "2.1: ",k,j,n,datout(k,:)
#endif
         ...
         where (datin(j,:).ne.xmissing)
           where (datout(k,:).eq.xmissing)
             datout(k,:) = datin(j,:)*(grdout(k+1)-grdin(j))
           elsewhere
             datout(k,:) = datout(k,:) + datin(j,:)*(grdout(k+1)-grdin(j))
           endwhere
         endwhere
#ifdef interp_weighted_scalar_2D
         print *, "2.2: ",k,j,n,"(grdout(k+1)-grdin(j))=",(grdout(k+1)-grdin(j))
         print *, "2.2: ",k,j,n,datin(j,:)
         print *, "2.2: ",k,j,n,datout(k,:)
#endif
       else if ( grdin(j)   >= grdout(k)   .and. &
                 grdin(j+1) <= grdout(k+1) ) then
         n=3
         where (datin(j,:).ne.xmissing)
           where (datout(k,:).eq.xmissing)
             datout(k,:) = datin(j,:)*(grdin(j+1)-grdin(j))
           elsewhere
             datout(k,:) = datout(k,:) + datin(j,:)*(grdin(j+1)-grdin(j))
           endwhere
         endwhere
#ifdef interp_weighted_scalar_2D
         print *, "2.3: ",k,j,n,"(grdin(j+1)-grdin(j))=",(grdin(j+1)-grdin(j))
         print *, "2.3: ",k,j,n,datin(j,:)
         print *, "2.3: ",k,j,n,datout(k,:)
#endif
       else if ( grdin(j)   <= grdout(k)   .and. &
                 grdin(j+1) >= grdout(k+1) ) then 
         n=4
         where (datin(j,:).ne.xmissing)
           where (datout(k,:).eq.xmissing)
             datout(k,:) = datin(j,:)*(grdout(k+1)-grdout(k))
           elsewhere
             datout(k,:) = datout(k,:) + datin(j,:)*(grdout(k+1)-grdout(k))
           endwhere
         endwhere
#ifdef interp_weighted_scalar_2D
         print *, "2.4: ",k,j,n,"(grdout(k+1)-grdout(k))=",(grdout(k+1)-grdout(k))
         print *, "2.4: ",k,j,n,datin(j,:)
         print *, "2.4: ",k,j,n,datout(k,:)
#endif
       endif
     enddo     
#ifdef interp_weighted_scalar_2D
     print *, "3.0: k=",k,datout(k,:)
     call flush()
#endif
     where (datout(k,:).ne.xmissing)
       datout(k,:) = datout(k,:)/(grdout(k+1)-grdout(k))  
     endwhere
#ifdef interp_weighted_scalar_2D     
     print *, "3.1: k=",k,datout(k,:)
     call flush()
#endif
   enddo
...
!!! subroutine interp_weighted_scalar_1D (grdin, grdout, datin, datout, verbose, mask_in, mask_out, &
!!!      missing_value, missing_permit, err_msg  )
   !-----------------------------------------------------------------------
   !   overload of interface interp_weighted_scalar_2D
   !   uses 2d arrays for data and mask
  ...
  do k = 1, size(datout(:))
    ...
    do j = 1, size(datin(:))
      if ( grdin(j)   <= grdout(k) .and. &
           ...
           grdin(j+1) <= grdout(k+1) ) then
        !!! datout(k) = datout(k) + datin(j)*(grdin(j+1)-grdout(k))
        if (datin(j).ne.xmissing) then
          if (datout(k).eq.xmissing) then
            datout(k) = datin(j)*(grdin(j+1)-grdout(k))
          else
            datout(k) = datout(k) + datin(j)*(grdin(j+1)-grdout(k))
          endif
         ...
         if (datin(j).ne.xmissing) then
           if (datout(k).eq.xmissing) then
             datout(k) = datin(j)*(grdout(j+1)-grdin(j))
           else
             datout(k) = datout(k) + datin(j)*(grdout(j+1)-grdin(j))
           endif
         endif
      else if ( grdin(j)   <= grdout(k)   .and. &
                grdin(j+1) >= grdout(k+1) ) then
...
!print '(a,2i3,4f6.1)', 'k,j=',k,j,grdout(k),grdin(j-1),grdin(j),wt
   ...
   ! wt = min(max(wt,0.),1.)
     if ((datin(j-1).ne.xmissing).and.(datin(j).ne.xmissing)) then
       datout(k) = (1.-wt)*datin(j-1) + wt*datin(j)
     else if (datin(j-1).ne.xmissing) then
       datout(k) = datin(j-1)
     else if (datin(j).ne.xmissing) then
       datout(k) = datin(j)
     else
       datout(k) = xmissing
     endif
...
use constants_mod, only: xmissing
