...
#################################################################
  ...
  set srcname    = src_clean
  set exename    = exec_clean
  set executable = fms_tikal.x
  set cppDefs    = (-Duse_libMPI -Duse_netCDF -DINTERNAL_FILE_NML -DSPMD -DUSE_LOG_DIAG_FIELD_INFO  -Duse_LARGEFILE -DP_SPLIT) 
  set upDates    = 
...
set fsDir    = $CTMP/$USER/$exp/$exename
...
#################################################################
  ...
  make OPENMP=ON -j 32 -f Makefile
   ...
   if ( $status ) then
     ...
     gcp $executable $fsDir/${executable}
